...
@section('content')
    <form class="col-6" action="{{ route('posts.update', $post) }}" method="post">
      ...
      <div class="form-group">
        ...
        <input name="title" type="text" class="form-control" id="formInputTitle" placeholder="post title" value="{{ $post->title }}">
      </div>
      <div class="form-group">
     ...
     <div class="form-group">
        ...
        <textarea name="content" class="form-control" rows="5" id="formInputContent">
          {{ $post->content }}
        </textarea>
      </div>
